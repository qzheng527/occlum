...
from solutions.antllm.antllm.arguments.glm_arguments import get_args
from solutions.antllm.antllm.data.configure_data import (
    build_multi_task_dataset, configure_data, get_split, prepare_tokenizer)
from solutions.antllm.antllm.datav2.data_utils import build_data_loader
from solutions.antllm.antllm.datav2.datasets import AutoDataset, split_dataset
from solutions.antllm.antllm.datav2.glm.batch_collator import GLMBlockCollator
from solutions.antllm.antllm.training.train_utils import (
    load_pretrained, setup_model_and_optimizer, train_step)
...
from solutions.antllm.antllm.utils.glm_utils import (Timers, get_log_dir,
                                                     get_sample_writer,
                                                     load_checkpoint,
                                                     print_and_save_args,
                                                     report_memory,
...
def get_masks_and_position_ids(
    ...
    if mem_length:
        ...
        attention_mask = torch.tril(
            torch.triu(attention_mask, 1 - seq_length + mem_length), mem_length
        )
    else:
...
def forward_step(data_iterator, model, args, timers, mems):
    ...
    rand = random.Random(
        args.iteration * mpu.get_data_parallel_world_size()
    ...
    def print_masked_text(batch_id):
        ...
        sep = (
            attention_mask.item()
            if torch.numel(attention_mask) == 1
            else attention_mask[batch_id].item()
        )
        text, last_segment = "", []
        for i, token_id in enumerate(tokens[batch_id, :sep].tolist()):
            ...
            if token.startswith("[MASK]") or token.endswith("[MASK]"):
                if last_segment:
                    text += tokenizer.decode(last_segment)
                    last_segment = []
        ...
        for i in range(sep, tokens.size(1)):
            if tokenizer.convert_ids_to_tokens(tokens[batch_id, i].item()).startswith(
                "<|startofpiece|>"
            ):
        ...
        if last_index is not None:
            print(
                tokenizer.decode(tokens[batch_id, last_index:].tolist()).encode(
                    "utf-8"
                ),
                "|",
                tokenizer.decode(labels[batch_id, last_index:].tolist()).encode(
                    "utf-8"
                ),
    ...
    model_output = model(tokens, position_ids, attention_mask, mems=mems, return_memory=False)
    logits, mems = model_output.logits, model_output.mems

    if args.atorch_accelerate:
        loss = model.atorch_loss_func(logits, labels, loss_mask)
    else:
...
def report_iteration_metrics(
    ...
    log_string += " iteration count: {} |".format(log_count)
    log_string += " learning rate {:.3E} |".format(lr)
    log_string += " lm loss {:.6E} |".format(loss)
    log_string += " bert loss {:.6E} |".format(all_losses[0])
    log_string += " sent loss {:.6E} |".format(all_losses[1])
    log_string += " gpt loss {:.6E} |".format(all_losses[2])
    if args.fp16:
        log_string += " loss scale {:.1f} |".format(
            optimizer.cur_scale if args.deepspeed else optimizer.loss_scale
        )
    elif args.atorch_accelerate:
        log_string += " loss scale {:.1f} |".format(optimizer.grad_scaler.get_scale())
    log_dist(log_string)
...
def train(
    ...
    # Tracking loss.
    total_lm_loss = 0.0
    total_bert_loss, total_sent_loss, total_gpt_loss = (
        0.0,
        0.0,
        0.0,
    )
    ...
    while args.iteration < args.train_iters:
        lm_loss, skipped_iter, mems, all_losses = train_step(
            train_data_iterator,
            model,
            optimizer,
            lr_scheduler,
        ...
        total_bert_loss += all_losses[0].data.detach().float()
        total_sent_loss += all_losses[1].data.detach().float()
        total_gpt_loss += all_losses[2].data.detach().float()

        # Logging.
        log_interval = 1 if args.iteration - start_iter <= 100 else args.log_interval
        if args.iteration % log_interval == 0:
            learning_rate = optimizer.param_groups[0]["lr"]
            avg_lm_loss = total_lm_loss.item() / log_interval
            avg_bert_loss = total_bert_loss.item() / log_interval
            avg_sent_loss = total_sent_loss.item() / log_interval
            avg_gpt_loss = total_gpt_loss.item() / log_interval
            elapsed_time, log_count = timers("interval time").elapsed()
            report_iteration_metrics(
                summary_writer,
                optimizer,
                learning_rate,
                avg_lm_loss,
                (avg_bert_loss, avg_sent_loss, avg_gpt_loss),
                elapsed_time * 1000.0 / log_interval,
                log_count,
                args.iteration,
                args.train_iters,
                args,
            )
            total_lm_loss, total_bert_loss, total_sent_loss, total_gpt_loss = (
                0.0,
                0.0,
                0.0,
                0.0,
...
def evaluate(data_iterator, model, args, timers, forward_step_func, verbose=False):
    ...
    with torch.no_grad():
        ...
        while iteration < args.eval_iters:
            ...
            if verbose and iteration % args.log_interval == 0:
                log_dist("Evaluating iter {}/{}".format(iteration, args.eval_iters))
            # Forward evaluation.
    ...
    total_lm_loss = (
        loss_data[0] / args.eval_iters / (args.world_size / args.model_parallel_size)
    )
...
def initialize_distributed(args):
    ...
    if (
        hasattr(args, "deepspeed")
        and args.deepspeed
        and args.deepspeed_activation_checkpointing
    ):
...
def build_datasetv2(args, tokenizer):
    collate_fn = GLMBlockCollator(tokenizer, args.seq_length)

    train_data = AutoDataset.from_config(
        args.train_data[0],
        tokenizer)
    args.do_train = True
    split = get_split(args)
    train_data, val_data, test_data = split_dataset(train_data, split, rank=get_rank())
    train_data = build_data_loader(
        train_data, args.batch_size * get_world_size(), args.train_iters, collate_fn,
        scatter_num=args.loader_scatter, shuffle=False, num_workers=2, pin_memory=True)
    if args.valid_data:
        val_data = AutoDataset.from_config(args.valid_data[0], tokenizer)
    if args.test_data:
        test_data = AutoDataset.from_config(args.test_data[0], tokenizer)

    if val_data:
        args.do_valid = True
        val_data = build_data_loader(
            val_data, args.batch_size * get_world_size(), args.train_iters, collate_fn,
            scatter_num=args.loader_scatter, shuffle=False, num_workers=2, pin_memory=True)

    if test_data:
        args.do_test = True
        test_data = build_data_loader(
            test_data, args.batch_size * get_world_size(), args.train_iters, collate_fn,
            scatter_num=args.loader_scatter, shuffle=False, num_workers=2, pin_memory=True)
...
def get_train_val_test_data(args, tokenizer):
    ...
    if mpu.get_model_parallel_rank() == 0:
        if args.datasetv2:
            train_data, val_data, test_data = build_datasetv2(args, tokenizer)
        else:
...
def main():
    ...
    (
        ...
        val_data,
        test_data,
    ) = get_train_val_test_data(args, tokenizer)
    ...
    model, optimizer, lr_scheduler, has_load = setup_model_and_optimizer(args)
    assert has_load is False, "not allow load from modelhub"
    ...
    if args.load_pretrained is not None:
        ...
        if args.atorch_accelerate and isinstance(model, FSDP):
            # atorch accelerate with fsdp need no filelock, but need entering concurrently
    ...
    if torch.distributed.get_rank() == 0:
        print("Start to begin training pretrain Model")
        args.log_dir = None
    ...
    if args.resume_dataloader:
        ...
        if multi_train_data is not None:
            multi_train_data.batch_sampler.start_iter = int(
                args.iteration * args.multi_task_ratio
            ) % len(multi_train_data)
        if multi_val_data is not None:
            start_iter_val = (
                (args.iteration // args.eval_interval)
                * args.eval_iters
                * args.multi_task_ratio
            )
