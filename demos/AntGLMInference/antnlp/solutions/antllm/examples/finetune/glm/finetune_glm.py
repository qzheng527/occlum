...
import examples.pretrain.glm.pretrain_glm as pretrain_glm
import torch
import torch.utils.data
from antllm.arguments.glm_arguments import get_args
from antllm.data.configure_data import make_data_loader, prepare_tokenizer
from antllm.data.data_utils import FakeDataloader, build_data_loader
from antllm.training.train_utils import (load_pretrained,
                                         setup_model_and_optimizer, train_step)
from antllm.utils import mpu
from antllm.utils.glm_utils import (Timers, get_log_dir, get_sample_writer,
                                    load_checkpoint, print_and_save_args,
                                    save_checkpoint)
from antllm.utils.logging.logger import log_dist
from examples.pretrain.glm.pretrain_glm import forward_step as lm_forward_step
from examples.pretrain.glm.pretrain_glm import (initialize_distributed,
...
def mix_forward_step(batch_and_dataloader, model, args, times, mems):
    ...
    if use_blocklm:
        return lm_forward_step(
            (batch_and_dataloader[1], None), model, args, times, mems
        )
    else:
    ...
    if args.pretrained_bert:
        ...
        if not args.fast_decode:
            ...
            if args.continuous_prompt:
                ...
                if args.hf_model:
                    model_output = model(tokens, position_ids, attention_mask, mems)
            ...
            dec_input_ids, dec_position_ids, dec_attention_mask = (
                data["dec_text"],
                data["dec_position"],
                data["dec_mask"],
            )
        ...
        tokens, labels, position_ids, attention_mask = (
            data["text"],
            data["label"],
            data["position"],
            data["mask"],
...
def _build_train_valid_dataloaders(train_dataset, valid_dataset, args):
    ...
    log_dist("building train and validation dataloaders ...")
    # Training dataset.
...
def _train(
    ...
    for epoch in range(start_epoch, args.epochs):
        log_dist("working on epoch {} ...".format(epoch))
        ...
        for iteration_, batch in enumerate(train_dataloader[0]):
            ...
            if do_export:
                print("start export")
                from alps.pytorch.modelhub.hub_layer import \
                    module_adjust_to_hublayer

                model_to_export = model.module.module.model
                hub_layer = module_adjust_to_hublayer(model_to_export)

                # torch.save(model)

                name = "glm-2b-ant-zh-s2"
                version = "v2"
                dt = "20230205"
                hub_layer.save_to_modelhub(
                    name=name,
                    version=version,
                    dt=dt,
                    # path请按照命名规则配置，以避免冲突
                    path=f"oss://alps-common/modelhub/layer/glm-2b-ant-zh-s1/v1/{dt}/{name}_{version}.tar.gz",
                )
            ...
            if (
                ...
                if eval_callback is not None:
                    score_dict = eval_callback(
                        model, args.iteration, summary_writer=summary_writer
                    )
                    if score_dict:
                        validation_metric = (
                            args.validation_metric
                            if args.validation_metric
                            else list(score_dict.keys())[0]
                        )
                        ...
                        if best_iteration is None or validation_score > best_score:
                            ...
                            if torch.distributed.get_rank() == 0:
                                ...
                                with open(
                                    os.path.join(args.log_dir, "results.json"), "w"
                                ) as output:
                                    output.write(json.dumps(score_dict) + "\n")
                                with open(
                                    os.path.join(
                                        args.save, "best_checkpointed_iteration.txt"
                                    ),
                                    "w",
                                ) as output:
    ...
    score_dict = eval_callback(model, args.iteration, summary_writer=summary_writer)
    if score_dict:
        validation_metric = (
            args.validation_metric
            if args.validation_metric
        ...
        if best_iteration is None or validation_score > best_score:
            ...
            if torch.distributed.get_rank() == 0:
                ...
                with open(os.path.join(args.log_dir, "results.json"), "w") as output:
                    output.write(json.dumps(score_dict) + "\n")
                with open(
                    os.path.join(args.save, "best_checkpointed_iteration.txt"), "w"
                ) as output:
                    output.write(str(best_iteration))
...
def finetune(
        ...
        if args.block_lm_ratio > 0.0:
            if mpu.get_model_parallel_rank() == 0:
                (
                    train_block_dataset,
                    valid_block_dataset,
                ) = train_valid_datasets_provider(args, tokenizer, pattern_text=True)
            ...
            train_block_dataloader, valid_block_dataloader = iter(
                train_block_dataloader
            ), iter(valid_block_dataloader)

    timers("train/valid/test dataset/dataloder").stop()
    # Build calback function.
    timers("callback function").start()
    eval_callback, end_of_train_callback = None, None
    if end_of_epoch_callback_provider is not None:
        if (
            train_valid_datasets_provider is not None
            and args.epochs > 0
            and not args.no_validation
        ):
            eval_callback = end_of_epoch_callback_provider(
    ...
    model, optimizer, lr_scheduler, has_load = setup_model_and_optimizer(
        args, **model_kwargs
    )
        ...
        if args.continuous_prompt and args.prompt_init:
            if mpu.get_model_parallel_rank() == 0:
                ...
                for label in processor.get_labels():
                    ...
                    verbalizer_ids = tokenizer.encode(verbalizer)
                    task_tokens += verbalizer_ids
                log_dist("Task tokens: " + tokenizer.decode(task_tokens))
    ...
    if torch.distributed.get_rank() == 0:
        ...
        if (
            os.path.exists(os.path.join(args.log_dir, "test_results.json"))
    ...
    if train_dataloader is not None and args.epochs > 0:
        ...
        if end_of_train_callback is not None:
            score_dict = end_of_train_callback(model, step=-1, output_predictions=True)
    # Or just evaluate.
    else:
        if end_of_train_callback is not None:
            log_dist("evaluation only mode, setting epoch to -1")
            score_dict = end_of_train_callback(model, step=-1, output_predictions=True)
