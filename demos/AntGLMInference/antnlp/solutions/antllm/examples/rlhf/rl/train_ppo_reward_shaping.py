...
if __name__ == "__main__":
    ...
    parser.add_argument("--cost_model_path", type=str, default=None)
    parser.add_argument(
        "--lambda_value",
        type=float,
        default=0.0,
    ...
    parser.add_argument("--rm_use_normalized_cost", action="store_true", default=False)
    ...
    # acotor tokenizer
    print(config.tokenizer.tokenizer_path)
    ...
    # load the pre-trained cost model
    cost_model, cost_tokenizer = load_pretrained_models(
        args.cost_model_path,
        num_head=args.num_head,
        rm_mean_value=args.rm_mean_value,
        use_position_id=args.rm_use_position_id,
        use_normalized_reward=args.rm_use_normalized_cost,  # cost 可能为pointwise
    )
    cost_model = init_eval_engine(
        model=cost_model,
        ds_config=ds_eval_config,
    )
    cost_model.eval()
    ...
    def reward_fn(samples: List[str], **kwargs):
        ...
        costs = get_scores_glm(
            ...
            max_length=config.train.seq_length,
            score_device=score_device,
            score_model=cost_model,
            truncation_side=truncation_side,
            eos_token=eos_token,
            rotary_type=cost_model.model.config.to_dict().get("rotary_type", "none"),
            sigmoid_reward=args.sigmoid_reward
        )
    ...
    def reward_fn_norm(samples: List[str], **kwargs):
        ...
        if args.repetition_penalty > 0.:
            ...
            for sample in samples:
                prompt, response = sample.split(args.mask_type, 1)
                repetition_rate = token_repetition_rate(rw_tokenizer, response, ngrams=[1])
                repetition_rates.append(repetition_rate)
            repetition_rates = torch.tensor(repetition_rates, device=scores.device)
            scores -= args.repetition_penalty * repetition_rates
        return scores
