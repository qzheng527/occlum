...
kmcli run --name tianxuan-glm10b-if-$(date "+%Y%m%d-%H%M%S") --runtime 'pytorch' --user 140807 --rdma --enable-host-network --image 'reg.docker.alibaba-inc.com/aii/aistudio:3750124-20230331223440' --master 'cpu=32,disk=102400,memory=819200,gpu=8,gpu_type=a100' --worker 'cpu=32,disk=102400,memory=819200,gpu=8,gpu_type=a100' $num_workers --app 'gbank' --priority=high --env PYTHONPATH=${output_dir},NCCL_DEBUG=INFO,NCCL_SOCKET_IFNAME=eth0,NCCL_IB_GID_INDEX=3,LD_LIBRARY_PATH=/usr/local/nvidia/lib:/usr/local/nvidia/lib64:/usr/local/cuda/lib64 "if [ ! -d /workspace ]; then mkdir /workspace; fi; mount -t nfs -o vers=3,nolock,proto=tcp alipay-heyuan-31-bmc76.cn-heyuan-alipay.nas.aliyuncs.com:/  /workspace && cd ${output_dir} && sh ${script} ${output_dir} ${resume_from_checkpoint}"
