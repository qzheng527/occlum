...
kmcli run --name "sft-a100-$USER-$resume_timestamp-`echo $RANDOM |cksum |cut -c 1-8`" \
--no-master \
--runtime 'elasticdl' \
--elasticdl-args "--distribution_strategy=AllreduceStrategy --relaunch_always" \
...
--app wulanchabu \
--user $USER \
"ldconfig && nvidia-smi && cd ${output_dir} && export PYTHONPATH=${output_dir} && sh ${script} ${output_dir} ${resume_from_checkpoint}"
