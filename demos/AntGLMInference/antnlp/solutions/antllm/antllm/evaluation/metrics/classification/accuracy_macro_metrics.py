...
class HuggingfaceAccuracyMacro():
    def __init__(self):
    ...
    def compute(self, predictions: List[List], references: List[List], extras: List[Dict]):
        ...
        for type, res in type_dict.items():
            ...
            type_acc_dict[type] = round(result["accuracy"] * 100, 2)
        results = sum(type_acc_dict.values()) / len(type_acc_dict)
        results = round(results, 2)
        return results


if __name__ == "__main__":
    test = HuggingfaceAccuracyMacro()
