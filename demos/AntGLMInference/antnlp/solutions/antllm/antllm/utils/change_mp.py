...
if iteration is not None:
    with open(
        os.path.join(new_checkpoint, "latest_checkpointed_iteration.txt"), "w"
    ) as fout:
        fout.write("{}\n".format(iteration))
    ...
    for i in range(target_mp):
        ...
        for j in range(start + 1, end):
            ...
            for k, v in d_new["module"].items():
                ...
                elif len(v.shape) == 1 and (
                    "dense_h_to_4h" in k or "attention.relative" in k
                ):
                    d["module"][k] = torch.cat([d["module"][k], v], 0)
        filename = os.path.join(
            new_checkpoint, "mp_rank_{:02d}_model_states.pt".format(i)
        )
...
if target_mp > len(filenames):
    ...
    for i in range(len(filenames)):
        ...
        for j in range(start, end):
                    ...
                    if len(v.shape) == 2 and "position" not in k:
                        ...
                        d_new["module"][k] = torch.cat(
                            [
                                v[shift * part : (shift + 1) * part].clone(),
                                v[
                                    (shift + ratio) * part : (shift + 1 + ratio) * part
                                ].clone(),
                                v[
                                    (shift + 2 * ratio)
                                    * part : (shift + 1 + 2 * ratio)
            ...
            filename = os.path.join(
                new_checkpoint, "mp_rank_{:02d}_model_states.pt".format(j)
            )
