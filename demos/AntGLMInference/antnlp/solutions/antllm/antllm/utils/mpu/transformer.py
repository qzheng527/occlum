    ...
    def forward(self, hidden_states, encoder_states, cross_mask):
        ...
        if self.flash_attn:
            ...
            if cross_mask is not None:
                ...
                attention_scores = torch.mul(attention_scores, cross_mask) - 10000.0 * (
                    1.0 - cross_mask
                )
...
class ParallelSelfAttention(torch.nn.Module):
    ...
    def forward(
        ...
        if mem is None:
            ...
            # print(f'================== mixed_x_layer: {mixed_x_layer.shape}')
    ...
    def forward(
        ...
        if self.checkpoint_activations:
            ...
            while x < num_layers:
                ...
                if memory_states:
                    args += memory_states[x: x + chunk_length]
                hidden_states = checkpoint(custom(x, x + chunk_length), *args)
                x += chunk_length
        else:
