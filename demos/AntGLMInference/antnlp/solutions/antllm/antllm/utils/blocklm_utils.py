...
if packaging.version.Version(np.version.version) >= packaging.version.Version("1.20"):
    setattr(np, "long", np.longlong)
...
class ConstructBlockStrategy:
    def __init__(
        ...
        self.rank = rank if rank is not None else get_data_parallel_rank()
        self.world_size = world_size if world_size is not None else get_data_parallel_world_size()
        # self.rank = 0
        # self.world_size = 1
        ...
        log_dist(
            f"""BERT prob {self.bert_prob},
            gap sent prob {self.gap_sentence_prob},
            GPT prob {self.gpt_prob},
            infill prob {self.infill_prob}"""
        )
        log_dist(
            f"""generation min ratio {self.gpt_min_ratio},
            block ratio {self.bert_ratio},
            gap sent ratio {self.gap_sentence_ratio}"""
        )
        log_dist(f"block length distribution {self.block_length_distribution}")
        log_dist(
            f"""block mask prob {self.block_mask_prob},
            context mask ratio {self.context_mask_ratio}"""
        )

    def contains_sentence_end(self, tok):
        tok = self.tokenizer.convert_ids_to_tokens(int(tok))
        if "." in tok:
            return True
        if "?" in tok:
            return True
            ...
            for _ in range(num_splits):
                if target_length >= len(tokens):
                    ...
                    while random_start > 0 and (
                        ...
                        or not (
                            self.contains_sentence_end(tokens[random_start - 1])
        ...
        if rand < self.bert_prob:
            ...
            for sample in samples:
                ...
                if not sentence_spans and torch.distributed.get_rank() == 0:
                    try:
                        print(self.tokenizer.decode(tokens[1:]))
                    except IndexError:
                        print(tokens[1:])
            ...
            for sample in samples:
                ...
                if self.use_prefix_mode or multiple_doc or rng.random() < self.infill_prob:
    ...
    def pad_batch(token_batch, target_batch, loss_mask_batch, position_id_batch):
        ...
        if seq_lengths.count(seq_lengths[0]) != len(seq_lengths):
            ...
            token_batch = [
                np.concatenate(
                    (tokens, np.zeros(max_length - len(tokens), dtype=np.long))
                )
                for tokens in token_batch
            ]
            target_batch = [
                np.concatenate(
                    (targets, np.zeros(max_length - len(targets), dtype=np.long))
                )
                for targets in target_batch
            ]
            loss_mask_batch = [
                np.concatenate(
                    (loss_masks, np.zeros(max_length - len(loss_masks), dtype=np.long))
                )
            ...
            position_id_batch = [
                np.concatenate(
                    (
                        position_ids,
                        np.zeros(
                            (2, max_length - position_ids.shape[1]), dtype=np.long
                        ),
                    ),
                    axis=1,
                )
