...
def get_delta_modified_modules(
    ...
    # TODO update glm freeze with num_layers_unfrozen
    if config.model_type == "glm":
        unfrozen_layers_pattern = generate_layer_regex(config, 6)
    else:
...
def is_peft_model_from_pretrained(pretrained_model_name_or_path: str):
    local_peft_adapter = os.path.exists(os.path.join(pretrained_model_name_or_path, "adapter_config.json"))
    if is_peft_available() and local_peft_adapter:
        return True
    else:
        return False