...
from antllm.data.configure_data import prepare_tokenizer
from antllm.utils import mpu
from antllm.utils.logging.logger import log_dist
from task.glm.arguments import get_args
from tasks.glm.glm_utils import (is_load_from_modelhub, load_checkpoint,
                                 parse_modelhub_path)
from tasks.glm.pretrain_glm import (get_masks_and_position_ids,
                                    initialize_distributed, set_random_seed)
from tasks.glm.train_utils import get_model
...
def setup_model(args):
    ...
    load_from_hub = (
        is_load_from_modelhub(args.load_pretrained) if args.load_pretrained else False
    )
    if load_from_hub:
        from alps.pytorch.modelhub.hub_layer import TorchHubLayer

        name, version, dt = parse_modelhub_path(args.load_pretrained)
        model = TorchHubLayer.restore_from_modelhub(name=name, version=version, dt=dt)
        print(f"load model from modelhub: {args.load_pretrained}")
        return model

    model, has_load = get_model(args, model_type="generation")
...
def get_batch(context_tokens, device, args):
    ...
    log_dist(f"attention_mask: {attention_mask}, position_ids: {position_ids}")
...
def sample_sequence(
    ...
    if not args.block_lm:
        ...
        tokens = torch.empty(
            (args.num_beams, 0), device=context_tokens.device, dtype=torch.long
        )
    else:
        tokens = context_tokens.new_full((1, 1), tokenizer.sop_token_id)
    counter = 0
    if mems is None:
        mems = []
    ...
    while counter < args.out_seq_length:
        if counter == 0 and not args.block_lm:
            next_token_logits, *mems = model(
                context_tokens, position_ids, attention_mask, *mems
            )
        else:
            if args.block_lm:
            ...
            last_token = tokens[:, -1:]
            next_token_logits, *mems = model(
                last_token, position_ids, attention_mask, *mems
            )
    ...
    if args.num_beams > 1:
        tokens, mems, _ = beam_scorer.finalize(
            tokens,
            beam_scores,
            next_tokens,
            next_indices,
...
def read_context(tokenizer, args, output):
    ...
    if mpu.get_model_parallel_rank() == 0:
        while True:
            ...
            if args.block_lm:
                context_tokens = [tokenizer.cls_token_id] + context_tokens
                if not raw_text.endswith("[gMASK]"):
                    context_tokens = context_tokens + [tokenizer.eos_token_id]
    ...
    if mpu.get_model_parallel_rank() != 0:
        raw_text = tokenizer.decode(context_tokens_tensor.tolist())
    ...
    with torch.no_grad(), open(output_path, "w") as output:
        while True:
            ...
            (
                ...
                context_tokens_tensor,
                context_length,
            ) = read_context(tokenizer, args, output)
            if terminate_runs == 1:
                return
            ...
            if args.block_lm:
                ...
                mask_tokens = [tokenizer.decode(token) for token in mask_tokens]
                end_tokens = [tokenizer.eop_token_id, args.eod_token]
                mask_positions = []
                for token in mask_tokens:
                    mask_positions += (
                        (context_tokens_tensor == token)
            ...
            if mpu.get_model_parallel_rank() == 0:
                ...
                trim_decode_tokens = postprocess_output(decode_tokens)
