...
if is_apex_available():
    from solutions.antllm.antllm.training.optimizer.fp16 import FP16_Optimizer
    from solutions.antllm.antllm.utils import mpu

SUMMARY_WRITER_DIR_NAME = "runs"
MODELHUB_PREFIX = "modelhub://"
...
def is_load_from_modelhub(load_path):
    return load_path.startswith(MODELHUB_PREFIX)


def parse_modelhub_path(load_path):
    assert is_load_from_modelhub(load_path)
    # FORMAT: modelhub://name/version/dt
    load_path = load_path[len(MODELHUB_PREFIX):]
    parts = load_path.split("/")
    assert len(parts) == 3
    name, version, dt = parts
    return name, version, dt
...
class Timers:
    ...
    def log(self, names, normalizer=1.0, reset=True):
        ...
        for name in names:
            ...
            total_elapse, count = self.timers[name].elapsed(reset=reset)
            elapsed_time = total_elapse * 1000.0 / count if count > 0 else 0.0
            string += " | {}: {:.2f}, {}".format(name, elapsed_time, count)
        log_dist(string)
...
def report_memory(name):
    ...
    string += " | max cached: {}".format(torch.cuda.memory_reserved() / mega_bytes)
    log_dist(string)
...
def save_zero_checkpoint(args, iteration, optimizer):
    zero_sd = {"iteration": iteration, "optimizer_state_dict": optimizer.state_dict()}
...
def save_checkpoint(
    ...
    elif args.atorch_accelerate and isinstance(model, FSDP):
        save_atorch_fsdp_checkpoint(
            iteration, model, optimizer, lr_scheduler, args, tag=tag
        )
    else:
        ...
        if mpu.get_data_parallel_rank() == 0:
            ...
            print(
                "global rank {} is saving checkpoint at iteration {:7d} to {}".format(
                    torch.distributed.get_rank(), iteration, checkpoint_name
                )
            )
...
def get_atorch_fsdp_misc_name(checkpoint_name):
    return os.path.join(
        os.path.dirname(checkpoint_name),
        f"mp_rank_{mpu.get_model_parallel_rank():02d}_misc.pt",
    )


def get_atorch_fsdp_osd_name(checkpoint_name, is_full=True):
    from atorch.distributed.distributed import (parallel_group_size,
                                                parallel_rank)

    fsdp_rank = parallel_rank("data")
    fsdp_world_size = parallel_group_size("data")
    if is_full:
        return os.path.join(
            os.path.dirname(checkpoint_name),
            f"mp_rank_{mpu.get_model_parallel_rank():02d}_full_osd.pt",
        )
    else:
        return os.path.join(
            os.path.dirname(checkpoint_name),
            f"mp_rank_{mpu.get_model_parallel_rank():02d}_"
            f"local_osd_{fsdp_rank}_in_{fsdp_world_size}.pt",
        )


def save_atorch_fsdp_checkpoint(iteration, model, optimizer, lr_scheduler, args, tag):
    from atorch.distributed.distributed import parallel_rank

    is_rank0 = parallel_rank("data") == 0
    checkpoint_name = get_checkpoint_name(args.save, tag)
    misc_name = get_atorch_fsdp_misc_name(checkpoint_name)
    if is_rank0:
        print(
            "## global rank {} is saving checkpoint at iteration {:7d} to {}".format(
                torch.distributed.get_rank(), iteration, checkpoint_name
    ...
    # fsdp save full state dict context, all rank involved
    ...
    if is_rank0:
        module_sd["module"] = full_sd

    if lr_scheduler is not None and is_rank0:
        sd["lr_scheduler"] = lr_scheduler.state_dict()

    # rng states.
    if not args.no_save_rng and is_rank0:
        sd["random_rng_state"] = random.getstate()
        sd["np_rng_state"] = np.random.get_state()
        sd["torch_rng_state"] = torch.get_rng_state()
        sd["cuda_rng_state"] = torch.cuda.get_rng_state()
    ...
    if is_rank0:
        ensure_directory_exists(checkpoint_name)
        ensure_directory_exists(misc_name)
        torch.save(module_sd, checkpoint_name)
        torch.save(sd, misc_name)
        print("  ## successfully saved {}".format(checkpoint_name))
        print("  ## successfully saved {}".format(misc_name))
    torch.distributed.barrier()

    # Optimizer stuff.
    if optimizer is not None:
        if args.atorch_save_fsdp_full_osd:
            # fsdp: all rank involved
            full_osd = FSDP.full_optim_state_dict(model, optimizer)
            if is_rank0:
                osd_name = get_atorch_fsdp_osd_name(checkpoint_name, is_full=True)
                print(
                    "global rank {} is saving full osd at iteration {:7d} to {}".format(
                        torch.distributed.get_rank(), iteration, osd_name
                    )
                )
                ensure_directory_exists(osd_name)
                torch.save(full_osd, osd_name)
                print("  successfully saved {}".format(osd_name))
            torch.distributed.barrier()
        else:
            local_osd = optimizer.state_dict()
            osd_name = get_atorch_fsdp_osd_name(checkpoint_name, is_full=False)
            print(
                "global rank {} is saving local osd at iteration {:7d} to {}".format(
                    torch.distributed.get_rank(), iteration, osd_name
                )
            )
            ensure_directory_exists(osd_name)
            torch.save(local_osd, osd_name)
            print("  successfully saved {}".format(osd_name))
            torch.distributed.barrier()
...
def get_checkpoint_iteration(load_path):
    ...
    if not os.path.isfile(tracker_filename):
        ...
        log_dist("    will not load any checkpoints and will start from " "random")
    ...
    with open(tracker_filename, "r") as f:
        ...
        #     release = metastring == 'release'
        #     if not release:
        #         log_dist('ERROR: Invalid metadata file {}. Exiting'.format(
        #             tracker_filename))
        #         exit()
...
def load_checkpoint(
    model, optimizer, lr_scheduler, args, no_deepspeed=False, no_load_optim=False
):
    """Load a model checkpoint."""

    load_dir, tag, release, success = get_checkpoint_iteration(args.load)
    # release = True#only for longer stage train
    print(
        f"load_dir: {load_dir}, tag: {tag}, release: {release}, " +
        "success: {success}, rank: {mpu.get_data_parallel_rank()}"
    )
    ...
    if args.deepspeed and not no_deepspeed:
        ...
        if not args.no_load_lr_scheduler and "client_lr_scheduler" in sd:
            ...
            log_dist("Load lr scheduler state")
        ...
        # if atorch accelerate, load misc/osd instead of module
        osd_name = None
        if args.atorch_accelerate and isinstance(model, FSDP):
            osd_name = get_atorch_fsdp_osd_name(
                checkpoint_name, is_full=args.atorch_load_fsdp_full_osd
            )
            misc_name = get_atorch_fsdp_misc_name(checkpoint_name)
            if os.path.isfile(misc_name):
                checkpoint_name = misc_name
        ...
        # if atorch accelerate, load module state dict before atorch accelerate
        ...
        if (
            ...
            try:
                if optimizer is not None:
                    if osd_name:
                        if args.atorch_load_fsdp_full_osd:
                            ...
                            if is_rank0:
                                print(
                                    "global rank {} is loading optim state dict {}".format(
                                        torch.distributed.get_rank(), osd_name
                                    )
                                )
                                osd = torch.load(osd_name, map_location="cpu")
                            else:
                                osd = None
                            torch.distributed.barrier()
                            osd = FSDP.scatter_full_optim_state_dict(osd, model)
                        else:
                            print(
                                "global rank {} is loading optim state dict {}".format(
                                    torch.distributed.get_rank(), osd_name
        ...
        if not args.no_load_lr_scheduler and (
            "client_lr_scheduler" in sd or "lr_scheduler" in sd
        ):
            if lr_scheduler is not None:
                # lr_sch saved in deepspeed as key ``client_lr_scheduler``
                lr_sch_sd = sd["lr_scheduler"] if "lr_scheduler" in sd else None
                if lr_sch_sd is None:
                    lr_sch_sd = (
                        sd["client_lr_scheduler"]
                        if "client_lr_scheduler" in sd
                        else None
                    )
                assert (
                    lr_sch_sd is not None
                ), "Both lr_scheduler and client_lr_scheduler is None"
                lr_scheduler.load_state_dict(lr_sch_sd)
    ...
    if not release and not args.finetune and not args.no_load_rng:
        try:
            ...
            # torch 2.1 cuda_rng_state size 816 -> 16, not set if shape inconsistent
            if sd["cuda_rng_state"].shape == torch.cuda.get_rng_state().shape:
...
def debug_finetune_data(local_vars, batch_id, tokenizer):
    ...
    attention_mask, logit_mask, position_ids = (
        local_vars["attention_mask"],
        local_vars["logit_mask"],
        local_vars["position_ids"],
    )
    output_tokens = []
    ...
    print(tokenizer.decode(tokens[batch_id][target_positions].tolist()))
    if len(target_ids.shape) > 2:
        print(tokenizer.decode(target_ids[batch_id][target_positions].tolist()))
    else:
        print(tokenizer.decode(target_ids[batch_id].tolist()))
