...
import torch
from antllm.data.data_utils import build_data_loader
from antllm.utils import mpu
from antllm.utils.logging.logger import log_dist
from examples.finetune.glm.finetune_glm import process_batch
...
def accuracy_func_provider(
    ...
    def metrics_func(model, step, output_predictions=False, summary_writer=None):
        log_dist("calculating metrics ...")
        score_dict = (
            OrderedDict([(key, 0.0) for key in metric_dict])
            if isinstance(metric_dict, dict)
            else {metric_dict: 0.0}
        )
        total = 0
        for name, dataloader in dataloaders:
            ...
            predictions, labels, examples = eval_func(
            ...
            output_str = " > |step: {}| metrics for {}: total {}".format(
                step, name, total_count
            )
            for key, value in single_dict.items():
                ...
                if (
                    ...
                    summary_writer.add_scalar(f"Train/valid_{name}_{key}", value, step)
        ...
        output_str = " >> |step: {}| overall: total = {}".format(step, total)
        for key, score in score_dict.items():
            output_str += " {} = {:.4f}".format(key, score)
            if summary_writer is not None and step >= 0 and not is_test:
                summary_writer.add_scalar(f"Train/valid_{key}", score, step)
        log_dist(output_str)
        return score_dict
...
def multichoice_evaluate(model, dataloader, example_dict, args):
    ...
    with torch.no_grad():
        ...
        for _, batch in enumerate(dataloader):
            ...
            if args.pretrained_bert:
                tokens, types, labels_, attention_mask = (
                    data["text"],
                    data["types"],
                    data["label"],
                    data["padding_mask"],
                ...
                tokens, labels_, position_ids, attention_mask = (
                    data["text"],
                    data["label"],
                    data["position"],
                    data["mask"],
    ...
    torch.distributed.all_gather_object(
        results_gathered, results, group=mpu.get_data_parallel_group()
    )
    results = {}
