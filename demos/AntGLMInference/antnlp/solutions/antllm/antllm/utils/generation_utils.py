...
class BeamSearchScorer(BeamScorer):
    ...
    def __init__(
        ...
        #     raise ValueError(
        #         f'''`num_beams` has to be an integer strictly greater than 1, 
        # but is {num_beams}. 
        ...
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            ...
            for beam_token_rank, (next_token, next_score, next_index) in enumerate(
                ...
                if (eos_token_id is not None) and (next_token.item() in eos_token_id):
                    ...
                    beam_hyp.add(
                        ...
                        next_score.item(),
                        mems=[mem[[next_index.item()]] for mem in mems]
                        if mems
                        else None,
                    )
                else:
            ...
            if beam_idx < self.num_beams:
                raise ValueError(
                    ...
                    can be equal to `eos_token_id: {eos_token_id}`. 
    ...
    def finalize(
        ...
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            ...
            for beam_id in range(self.num_beams):
                ...
                beam_hyp.add(
                    final_tokens,
                    final_score,
                    mems=[mem[[batch_beam_idx]] for mem in mems] if mems else None,
                )
        ...
        for i, (hypo, mem, score) in enumerate(best):
            ...
            mems.append(mem)
        mems = (
            [torch.cat([mem[i] for mem in mems], dim=0) for i in range(len(mems[0]))]
            if mems and mems[0]
            else None
        )
...
class NoRepeatNGramLogitsProcessor(LogitsProcessor):
    ...
    def __call__(
        ...
        num_batch_hypotheses = scores.shape[0]  # bsz*beam_size
        cur_len = input_ids.shape[-1]
