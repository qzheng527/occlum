...
from solutions.antllm.antllm.data import data_utils
from solutions.antllm.antllm.data.samplers import (
...
class MultiTaskDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.weights = np.array([min(x, max_limit) ** temperature for x in self.lens])
        self.total_len = sum(self.lens)
        self.cumulative_lens = list(accumulate(self.lens))
        if self.reweight:
            log_dist(list(zip(self.tasks, self.lens, self.weights)))
        else:
            log_dist(list(zip(self.tasks, self.lens)))
...
def prepare_tokenizer(args):
    tokenizer = GLMTokenizer.from_pretrained(args.spm_tokenizer_path)
    if mpu.get_model_parallel_rank() == 0:
        num_tokens = tokenizer.vocab_size
        eod_token = tokenizer.eos_token_id
        assert eod_token == tokenizer.pad_token_id
        before = num_tokens
        after = before
        ...
        log_dist("> found end-of-document token: {}".format(eod_token))
...
def make_data_loader(
    ...
    if args.transformer_xl:
        batch_sampler = DistributedSequentialSampler(
    ...
    if block_collate:
        collate_fn = ConstructBlockStrategy(
            ...
            use_prefix_mode=args.use_prefix_mode
        ).construct_blocks
...
def make_loaders(args, tokenizer):
    ...
    if args.loader_scatter is not None:
        assert (
            world_size % args.loader_scatter == 0
        ), f"world_size: {world_size} must large or equal loader_scatter: {args.loader_scatter}"
