...
def exists_lazy(path, data_type='data'):
    ...
    if not os.path.exists(get_lazy_path(path)):
        print(f'missing {get_lazy_path(path)}')
        return False
    contents = os.listdir(get_lazy_path(path))
    if data_type not in contents:
        print(f'missing {data_type} in {get_lazy_path(path)}')
        return False
    if data_type + '.lens.npy' not in contents:
        print(f'missing {data_type}.lens.npy in {get_lazy_path(path)}')
        return False
    return True
