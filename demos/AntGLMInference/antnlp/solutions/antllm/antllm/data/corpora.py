...
class DataReader:
    ...
    def __init__(self, writers, tokenizer=None, tokenize=False, **kwargs):
        ...
        log_dist(f"Creating dataset from {self.PATH}")
        self.tokenizer = tokenizer
    ...
    def process_sample(cls, text, tokenizer, tokenize):
        if isinstance(text, str) and tokenize:
            ...
            text = tokenizer.encode(text) if text else []
        return text
...
class KeyReader(DataReader):
    ...
    def process_line(self, data, tokenizer, tokenize):
        ...
        keys = [tokenizer.encode(key) for key in keys]
        contents = [tokenizer.encode(content) for content in contents]
        summary = sum(keys, [])
        ...
        for key, content in zip(keys, contents):
            content = content + [tokenizer.eop_token_id]
            text += key
            text += content
...
class zhihu(PromptReader):
    ...
    def process_line(self, data, tokenizer, tokenize):
        ...
        if ans_length > 100 or ans_up > 1000:
            ...
            prompt = self.qtitle_prefix + qtitle + self.qcontent_prefix + \
                qcontent + self.user_prefix + user + self.answer_prefix
...
class zhidao(PromptReader):
    ...
    def process_line(self, data, tokenizer, tokenize):
        ...
        qcontent = self.trim_field(qcontent, max_length=100)
        prompt = self.qtitle_prefix + qtitle + \
            self.qcontent_prefix + qcontent + self.answer_prefix
...
class baike(PromptReader):
    ...
    def process_line(self, data, tokenizer, tokenize):
        ...
        text = data.get("title", "") + data.get("abstract",
                                                "") + data.get("content", "")
        if text:
            p, t = self.process_sample("", tokenizer, tokenize), self.process_sample(
...
class TestDataset(PromptReader):
    ...
    def process_line(self, data, tokenizer, tokenize):
        ...
        prompt, text = self.process_sample(
            prompt, tokenizer, tokenize), self.process_sample(text, tokenizer, tokenize)
        return [prompt], [text]
...
class WuDaoMiniCorpus(WuDaoCorpus):
    PATH = "corpus/wudao_mini"


class WuDaoBlankCorpus(WuDaoCorpus):
    PATH = "corpus/wd_blank"
...
NAMED_CORPORA = {
    ...
    'stories': Stories,
    'wudao': WuDaoCorpus,
    'wd_blank': WuDaoBlankCorpus,
    'wudao_mini': WuDaoMiniCorpus,
    'base': wikipedia,
    'zh_cc': Cc,
    'yq': Yq,
    'cc_blank': CcBlank,
