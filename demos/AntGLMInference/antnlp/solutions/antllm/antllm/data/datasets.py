...
class ShuffleDataset(data.Dataset):
    def __init__(self, ds):
        ...
        if self.is_lazy:
            self.prompt_lens = [self.ds.prompt_lens[idx]
                                for idx in self.shuffle_ids]
            self.text_lens = [self.ds.text_lens[idx]
                              for idx in self.shuffle_ids]
        ...
        if self._lens is None:
            ...
            if self.is_lazy:
                for x in self.datasets:
                    self._lens.extend(x.lens)
            else:
                for x in self.datasets:
                    self._lens.extend([len(d['text']) if isinstance(
                        d, dict) else len(d) for d in x])
        return self._lens
...
class SplitDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        # log_dist(f'get_item. index: {index}, {len(self.split_inds)}, {len(self.wrapped_data)}')
    ...
    def __init__(self, path, tokenizer=None, preprocess_fn=None, delim=',',
        ...
        try:
            ...
            if isinstance(label_key, list):
                ...
                cols += [label_key]
            data = pd.read_csv(self.path, sep=self.delim,
                               usecols=cols, encoding='latin-1')
        except Exception as e:
            print(str(e))
            data = pd.read_csv(self.path, sep=self.delim, usecols=[
                               text_key], encoding='latin-1')
    ...
    def write(self, writer_gen=None, path=None, skip_header=False):
        ...
        with open(path, 'w') as csvfile:
            ...
            if writer_gen is not None:
                ...
                if not skip_header:
                    header = (self.label_key,) + \
                        tuple(next(writer_gen)) + (self.text_key,)
                    c.writerow(header)
...
class XLDataset(data.Dataset):
    ...
    def init_indices(self):
        if self.is_lazy:
            lens = np.array([self.ds.get_text_len(idx)
                            for idx in range(len(self.ds))])
        else:
            lens = np.array([len(d['prompt']) + len(d['text'])
                            if isinstance(d, dict) else len(d) for d in self.ds])
    ...
    def getidx(self, idx):
        ...
        while len(tokens) < self.max_seq_len and sample_idx < len(self.ds):
            ...
            end = min(len(text) - 1, token_offset +
    ...
    def pad_seq(self, seq, pad_id=None):
        ...
        seq += [self.tokenizer.pad_token_id if pad_id is None else pad_id] * (num_pad_tokens)
        return seq
...
class BlockDataset(data.Dataset):
    def __init__(self, ds, tokenizer,
        ...
        if self.filter_english:
            ...
            log_dist("Load language detection model")
        if hasattr(self.ds, 'is_lazy') and self.ds.is_lazy:
            self.is_lazy = True
        self.init_weighting()
    ...
    def init_weighting(self):
        if self.is_lazy:
            ...
            lens = np.array([len(d['text']) if isinstance(
                d, dict) else len(d) for d in self.ds])
        self.total_len = np.sum(lens)
        self.weighting = np.cumsum(lens)
        log_dist(
            f'''Dataset document count {len(lens)},
            token count {self.total_len}, non sentence start{self.non_sentence_start},
            self.weighting: [{self.weighting[-10:]}],
            len: {self.weighting.shape}''')
    ...
    def __getitem__(self, idx):
        ...
        if tokens_to_strip > 0:
            ...
            if rng.random() > self.non_sentence_start:
                if rng.random() < 0.5:
                    while move_count < self.max_seq_len // 2 and  \
                        strip_left_tokens > 0 and \
                            not self.contains_sentence_end(tokens[strip_left_tokens - 1]):
                        strip_left_tokens -= 1
                        move_count += 1
                else:
                    while move_count < self.max_seq_len // 2 and \
                        strip_left_tokens < len(tokens) and \
                            not self.contains_sentence_end(tokens[strip_left_tokens - 1]):
                        strip_left_tokens += 1
                        move_count += 1
            tokens = [self.tokenizer.cls_token_id] + tokens[strip_left_tokens:]
            loss_mask = [0] + loss_mask[strip_left_tokens:]
            if len(tokens) == 2 and tokens[1] == self.tokenizer.eos_token_id:
                tokens, loss_mask = [], []
            tokens, loss_mask = self.right_strip_seq(
                tokens, loss_mask, self.max_seq_len)
        else:
            ...
            if self.sample_across_doc:
                while len(tokens) < self.max_seq_len:
                    ...
                    new_tokens = [self.tokenizer.cls_token_id] + new_tokens
...
class BertSentencepairDataset(data.Dataset):
    ...
    Arguments:
        ...
        max_preds_per_seq (int): Maximum number of masked tokens per sentence pair.
        Default: math.ceil(max_seq_len*mask_lm_prob/10)*10
    ...
    def __init__(self, ds, max_seq_len=512, mask_lm_prob=.15, max_preds_per_seq=None, short_seq_prob=.01,
        ...
        self.vocab_words = list(self.tokenizer.get_vocab().values())
    ...
    def __getitem__(self, idx):
        ...
        tokensa, tokensb = self.truncate_seq_pair(
            tokensa, tokensb, self.max_seq_len, rng)
    ...
    def sentence_tokenize(self, sent, sentence_num=0, beginning=False, ending=False):
        ...
        token_types = [int(sentence_num)] * len(tokens)
    ...
    def mask_token(self, idx, tokens, types, vocab_words, rng):
        ...
        label = tokens[idx]
        if rng.random() < 0.8:
            new_label = self.tokenizer.mask_token_id
        else:
            if rng.random() < 0.5:
                new_label = label
            else:
    ...
    def pad_seq(self, seq):
        ...
        seq += [self.tokenizer.pad_token_id] * num_pad
        return seq, pad_mask
    ...
    def create_masked_lm_predictions(self, a, b, mask_lm_prob, max_preds_per_seq, vocab_words, rng):
        ...
        tokens_a, token_types_a = a
        tokens_b, token_types_b = b
        tokens = [self.tokenizer.cls_token_id] + tokens_a + [
            self.tokenizer.sep_token_id] + tokens_b + [self.tokenizer.sep_token_id]
        ...
        cand_indices = [
            idx + 1 for idx in range(len_a)] + [idx + 2 + len_a for idx in range(len_b)]
        ...
        num_to_predict = min(max_preds_per_seq, max(
            1, int(round(len(tokens) * mask_lm_prob))))
        ...
        for idx in sorted(cand_indices[:num_to_predict]):
            mask[idx] = 1
            label = self.mask_token(
                idx, output_tokens, output_types, vocab_words, rng)
