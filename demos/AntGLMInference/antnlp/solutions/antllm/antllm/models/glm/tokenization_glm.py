...
class GLMTokenizerMixin:
    ...
    def decode_whitespaces(self, content):
        ...
        # pattern = re.compile('<\|blank_[0-9]{1,2}\|> ')
        # matches = re.findall(pattern, content)
        # for match in matches:
        #     content = content.replace(match, match[:-1])
        for i in range(50, 1, -1):
...
class GLMChineseTokenizer(PreTrainedTokenizer, GLMTokenizerMixin):
    vocab_files_names = {"vocab_file": "merge.model"}
    truncation_side: str = "left"
    ...
    def check_special_tokens(self):
        ...
        for name, special_token in zip(
            ['eos', 'unk', 'sep', 'pad', 'cls', 'mask'],
            [
                self.eos_token, self.unk_token, self.sep_token,
    ...
    def tokenize(self, text: TextInput, **kwargs) -> List[str]:
        ...
        # keep consistant with pretrained model tokenize
        for i, token in enumerate(tokens):
            if token in ['[MASK]', '[gMASK]', '[sMASK]']:
        ...
        **kwargs,
        ...
        for i in range(len(token_ids)):
            if token_ids[i] in self.all_special_ids:
                if i > last:
                    sub_texts.append(self._decode(
                        token_ids=token_ids[last: i],
                        skip_special_tokens=skip_special_tokens,
                        clean_up_tokenization_spaces=clean_up_tokenization_spaces,
                        spaces_between_special_tokens=spaces_between_special_tokens,
                        **kwargs)
                    )
                sub_texts.append(self._convert_id_to_token(token_ids[i]))
                last = i + 1
        if last < len(token_ids):
            sub_texts.append(self._decode(
                token_ids=token_ids[last:],
                skip_special_tokens=skip_special_tokens,
                clean_up_tokenization_spaces=clean_up_tokenization_spaces,
                spaces_between_special_tokens=spaces_between_special_tokens,
                **kwargs)
            )
        decode_text = ''.join(sub_texts)
