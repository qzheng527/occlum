...
from solutions.antllm.antllm.utils.glm_utils import get_hostname
...
def add_model_config_args(parser):
    ...
    group.add_argument(
        "--transformer-xl", action="store_true", help="use transformer-xl for training"
    )
    group.add_argument(
        "--pretrained-bert",
        action="store_true",
    ...
    group.add_argument(
        ...
        type=int,
        default=0,
        help="num of transformer key value heads, default is 0 which means MHA",
    )
    group.add_argument(
    ...
    group.add_argument(
        "--flash_attn", action="store_true", help="Use Atorch flash attention."
    )
    group.add_argument(
        "--hf-model", action="store_true", help="Get model from huggingface version."
    )
    group.add_argument(
        "--atorch-accelerate",
        action="store_true",
        help="Use ATorch auto_accelerate for hf glm.",
    )
    group.add_argument(
        "--atorch-meta-init",
        action="store_true",
        help="Use ATorch meta init for large model init.",
    )
    group.add_argument(
        "--atorch-load-fsdp-full-osd",
        action="store_true",
        help="Load with fsdp full optimizer state dict.",
    )
    group.add_argument(
        "--atorch-save-fsdp-full-osd",
        action="store_true",
        help="Save with fsdp full optimizer state dict.",
    )
...
def add_training_args(parser):
    ...
    group.add_argument("--local-rank", type=int, default=-1)
    ...
    group.add_argument(
        "--save-epoch", type=int, default=1, help="number of epochs between saves"
    )
    group.add_argument(
        "--save-interval",
        type=int,
    ...
    group.add_argument(
        "--block-lm", action="store_true", help="whether use the BlockLM pre-training"
    )
    group.add_argument(
    ...
    group.add_argument(
        "--use-prefix-mode",
        action="store_true",
        help="Use prefix mode to prepare gmask task",
    )
...
def add_evaluation_args(parser):
    ...
    group.add_argument(
        "--max-eval-steps", type=int, default=10000000, help="max steps to evaluation"
    )
    group.add_argument(
        "--eval-epoch",
        type=int,
...
def add_data_args(parser):
    ...
    group.add_argument("--datasetv2", action="store_true", help="Whether to use v2 dataset")
    group.add_argument(
        "--train-data",
        nargs="+",
        default=None,
    ...
    group.add_argument(
        "--data-dir", type=str, default=None, help="The data path to all the data files"
    )
    group.add_argument(
...
def add_finetune_config_args(parser):
    ...
    group.add_argument(
        "--load-pretrained", type=str, help="Load pretrained model", default=None
    )
    group.add_argument(
        "--pool-token",
        type=str,
    ...
    group.add_argument(
        "--cloze-eval", action="store_true", help="Evaluation dataset with cloze task"
    )
    group.add_argument(
        "--multi-token",
        action="store_true",
    ...
    group.add_argument(
        "--eval-valid", action="store_true", help="Whether evaluate on the valid set"
    )
    ...
    group.add_argument(
        "--continuous-prompt", action="store_true", help="Use continuous prompt for PET"
    )
...
def get_args():
    ...
    if (
        ...
        if "optimizer" in deepspeed_config:
            ...
            args.adam_eps = optimizer_params_config.get("eps", args.adam_eps)
            args.adam_beta1, args.adam_beta2 = optimizer_params_config.get(
                "betas", [args.adam_beta1, args.adam_beta2]
            )
    if hasattr(args, "atorch_accelerate") and args.atorch_accelerate:
        # handle in auto_accelerate
        args.checkpoint_activations = False
        args.fp16 = False
        if hasattr(args, "deepspeed"):
            args.deepspeed = False
    else:
        if hasattr(args, "atorch_meta_init"):
            args.atorch_meta_init = False
    return args
...
def mpi_define_env(args):
    ...
    # NODE_SIZE and NPROC_PER_NODE
    nproc_per_node = sum([i == proc_name for i in all_procs])
    node_size = len(set(all_procs))
    ...
    print(
        ...
        + "master_addr={}, master_port={}, node_size={}, nproc_per_node={}".format(
            os.environ["RANK"],
            args.local_rank,
            os.environ["WORLD_SIZE"],
            os.environ["MASTER_ADDR"],
            os.environ["MASTER_PORT"],
            os.environ["NODE_SIZE"],
