...
def exists_lazy(path, data_type='data'):
    ...
    if not os.path.exists(get_lazy_path(path)):
        log_dist(f'missing {get_lazy_path(path)}')
        return False
    contents = os.listdir(get_lazy_path(path))
    if data_type not in contents:
        log_dist(f'missing {data_type} in {get_lazy_path(path)}')
        return False
    if data_type + '.lens.npy' not in contents:
        log_dist(f'missing {data_type}.lens.npy in {get_lazy_path(path)}')
        return False
    return True
...
def exists_scatter(path, scatter_num=64, data_type='data'):
    for i in range(scatter_num):
        ...
        if not exists_lazy(scatter_path, data_type=data_type):
            log_dist(f'missing {scatter_path}')
            return False
    return True
...
class LazyWriter:
    def __init__(self, path, data_type, is_array=False, array_data_type=np.int32):
        ...
        self.lenpath = os.path.join(lazypath, data_type + '.lens.npy')
    ...
    def get_len_path(path, data_type):
        lazypath = get_lazy_path(path)
        return os.path.join(lazypath, data_type + '.lens.npy')
...
class LazyLoader(object):
    ...
    def __init__(self, path, data_type='data', mem_map=False, map_fn=None, is_array=False, array_data_type=np.int32,
        ...
        lenpath = os.path.join(lazypath, data_type + '.lens.npy')
        self.lens = np.load(lenpath)
        if half_load:
            self.lens = self.lens[:2 * len(self.lens) // 3]
        self.ends = np.cumsum(self.lens)
        # self.dumb_ends = list(self.ends)
        self.mem_map = mem_map
        self.load_memory = load_memory
        self.load_memory = False
        print(
            f'load_memory: {self.load_memory}, mem_map: {self.mem_map}, datapath: {datapath}')
        if self.load_memory:
            ...
            self.file = np.ndarray(shape=(len(self.file) // data_type_size,), dtype=array_data_type, buffer=self.file,
                                   order='C')
            print(f'load_memory: {self.file.shape}, path: {datapath}')
        elif self.mem_map:
            if is_array:
                if self.ends[-1] == 0:
                    ...
                    self.file = np.memmap(
                        datapath, dtype=array_data_type, mode='r', order='C')
                    self.file = memoryview(self.file)
            else:
                if self.ends[-1] == 0:
                    self.file = bytearray()
                else:
    ...
    def file_read(self, start=0, end=None):
        ...
        if not self.mem_map and not self.load_memory:
            ...
            dlen = end - start
            offset = start * 4
            rtn = np.frombuffer(self.file, dtype=np.int32,
                                count=dlen, offset=offset)
            if self.is_array:
                rtn = rtn.copy()
            else:
