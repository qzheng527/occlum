...
import numpy as np
import torch
from antllm.data.data_utils import (build_input_from_ids,
...
class LMDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.num_sequences = [
            max(math.ceil(target / self.overalapping_eval) + 1, 1) for target in targets
        ]
...
class LambadaDataset(torch.utils.data.Dataset):
    def __init__(self, args, tokenizer, strict=True):
        ...
        log_dist("> building lambada dataset from {} ...".format(data_path))
        self.args = args
    ...
    def get_tokens(self, text):
        ...
        beginning_tokens = self.tokenizer.encode(
    ...
    def __getitem__(self, idx):
        ...
        if self.block_lm:
            ...
            num_special_tokens = num_special_tokens_to_add(
                tokens, None, answer_tokens, add_cls=True, add_sep=False, add_piece=True
            )
            left_shift = (
                len(tokens)
                + len(answer_tokens)
...
def build_lambada_dataset(tokenizer, args):
    ...
    log_dist(
        " > found {} samples, {} label tokens.".format(
...
def build_wikitext103_dataset(tokenizer, args):
    ...
    log_dist(entire_data[:1024])
    tokenized_data = tokenizer.encode(entire_data)
    num_tokenized_tokens = len(tokenized_data)

    val_dataset = LMDataset(
        args, [tokenized_data], tokenizer, num_original_tokens, num_tokenized_tokens
    )
    log_dist(
