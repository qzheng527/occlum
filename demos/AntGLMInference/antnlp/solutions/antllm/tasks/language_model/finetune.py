    ...
    def print_masked_text(batch_id):
        ...
        for i, token in enumerate(tokens[batch_id, :sep].tolist()):
            if global_tokenizer is not None:
                token = global_tokenizer.convert_ids_to_tokens(token)
        ...
        for i in range(sep, tokens.size(1)):
            if global_tokenizer.convert_ids_to_tokens(tokens[batch_id, i].item()).startswith(
                "<|startofpiece|>"
            ):
                if last_index is not None:
                    print(
                        global_tokenizer.decode(
        ...
        if last_index is not None:
            print(
                global_tokenizer.decode(tokens[batch_id, last_index:].tolist()),
                "|",
                global_tokenizer.decode(labels[batch_id, last_index:].tolist()),
            )
            print(
    ...
    with torch.no_grad():
        ...
        for iteration, batch in enumerate(dataloader):
            if (iteration + 1) % args.log_interval == 0:
                log_dist("> working on iteration: {}".format(iteration))
            # Forward evaluation.
            output, _, _ = lm_forward_step(
...
def metrics_func_provider(args, tokenizer, is_test):
    ...
    dataloader = build_data_loader(
        dataset, args.eval_batch_size, args.num_workers, drop_last=False, shuffle=False
    )
