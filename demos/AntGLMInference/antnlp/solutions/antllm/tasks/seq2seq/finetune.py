...
from antllm.utils.eval_utils import accuracy_func_provider
from antllm.utils.logging.logger import log_dist
from examples.finetune.glm.finetune_glm import finetune
from examples.pretrain.glm.pretrain_glm import get_batch
from tasks.seq2seq.dataset import (BlankLMDataset, CustomizationDataset,
                                   ExtractionDataset, Seq2SeqDataset)
...
def metrics_func_provider(args, tokenizer, is_test):
    ...
    if args.task.lower() in ["blank", "extraction"]:
        ...
        if args.tokenizer_type == "BertWordPieceTokenizer":
            ...
            dataset = "cnn_dm_org"
        if args.task.lower() in ["squad", "squad_v1", "cmrc"]:
            metric_dict = {"EM": squad_exact_match, "F1": squad_f1}
        else:
            metric_dict = OrderedDict(
                {
                    "rouge-1": functools.partial(
                        rouge_metric, metric="rouge-1", dataset=dataset
                    ),
                    "rouge-2": functools.partial(
                        rouge_metric, metric="rouge-2", dataset=dataset
                    ),
                    "rouge-l": functools.partial(
                        rouge_metric, metric="rouge-l", dataset=dataset
                    ),
                }
            )
    ...
    def output_func(predictions, examples, output_file):
        if args.task.lower() in ["squad", "squad_v1", "cmrc"]:
            ...
            log_dist(f"write predictions to {output_file}")
    ...
    log_dist(f"using metric {metric_dict.keys()}")
